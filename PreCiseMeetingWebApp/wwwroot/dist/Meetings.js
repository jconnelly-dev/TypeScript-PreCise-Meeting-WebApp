...
export class Meetings {
    static getWeeklyMeetingsInYear(date, names) {
        ...
        const meetingDayOfWeek = DateHelper.getDayOfWeek(date);
        ...
        const dayInYear = date.getDate();
        const totalDaysInYear = DateHelper.getNumDaysInYear(date);
        ...
        for (let day = 1; day <= daysRemainingInYear; day++) {
            const nextDate = new Date(year, 0, dayInYear + day);
        ...
        const numFirstPersonMeetingsInYear = (totalMeetingsInYear % names.length == 0) ? totalMeetingsInYear / names.length : (totalMeetingsInYear / names.length) + 1;
        ...
        do {
            ...
            const groupedMeetings = Randomizer.getUniqueFirstPersonGroups(firstMeetingOfYear, names);
            if (groupedMeetings) {
                for (let groupMeetingIdx = 0; groupMeetingIdx < groupedMeetings.length && currMeetingIdx < meetings.length; groupMeetingIdx++) {
    ...
    static getMeetingForWeek(date, names) {
        Logger.log(Logger.LogLevel.INFO, Meetings, Meetings.getMeetingForWeek);
        let randomizedNames = new Array();
        try {
            if (!names) {
                throw new PreCiseException("names given for weekly meeting is null or empty.");
            }
            const meetingDayOfWeek = DateHelper.getDayOfWeek(date);
            const weekNumberInYear = DateHelper.getDayOfWeekOfYear(date, meetingDayOfWeek); // 1-based number
            ...
            const meetings = this.getWeeklyMeetingsInYear(date, names);
            ...
            const meetingForWeek = meetings.find(meeting => meeting.id === weekNumberInYear);
            if (!meetingForWeek) {
